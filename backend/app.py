...
def pdf_to_word():
    ...
    try:
        ...
        out = OUTPUTS / f"{safe_stem}_toolmitra.docx"

        doc = Document()
        doc.add_heading('Converted PDF Text', level=1)
        doc.add_paragraph('Generated by ToolMitra. This mode is optimized for large PDFs and mobile browsers.')

        page_count = 0
        extracted_any = False

        # Primary: PyMuPDF is fastest and safest for large PDFs.
        if fitz is not None:
            try:
                with fitz.open(str(pdf)) as pdf_doc:
                    ...
                    for page_no in range(page_count):
                        ...
                        doc.add_heading(f'Page {page_no + 1}', level=2)
                        if text.strip():
                            extracted_any = True
        ...
        # Fallback: pypdf simple text extraction (lighter than pdfplumber).
        if page_count == 0:
            try:
                ...
                traceback.print_exc()
                return jsonify(error='Could not read this PDF. It may be damaged, scanned-only, or password protected.'), 400

        if page_count == 0:
            return jsonify(error='Could not read any page from this PDF.'), 400

        if not extracted_any:
            doc.add_paragraph('No selectable text was found. This looks like scanned/image PDF. Use OCR PDF first, then convert to Word.')

        doc.save(out)
        return send_file(out, as_attachment=True, download_name=out.name, mimetype='application/vnd.openxmlformats-officedocument.wordprocessingml.document')
...
def image_compress():
    ...
    def save_jpg(im, q):
        im = im.convert('RGB')
        im.save(out, 'JPEG', quality=max(5, min(95, int(q))), optimize=True, progressive=True)

    def save_webp(im, q):
        im.save(out, 'WEBP', quality=max(5, min(100, int(q))), method=6)
    ...
    if not target_kb_raw:
        if fmt == 'jpg':
            save_jpg(img, quality)
        elif fmt == 'webp':
            save_webp(img, quality)
        else:
            save_png(img)
        return send(out, out.name)
    ...
    def save_quality(im, q):
        if fmt == 'jpg':
            save_jpg(im, q)
        elif fmt == 'webp':
            save_webp(im, q)
        else:
            im2 = im.convert('P', palette=Image.ADAPTIVE, colors=128)
            im2.save(out, 'PNG', optimize=True, compress_level=9)
    ...
    for attempt in range(35):
        ...
        for _ in range(10):
            mid = (lo + hi) // 2
            save_quality(work, mid)
            size = out.stat().st_size

            if size <= limit:
                local_best = out.read_bytes()
                local_size = size
                lo = mid + 1
            else:
                hi = mid - 1
        ...
        if work.width <= 120 or work.height <= 120:
            break

        scale = 0.90
        nw = max(120, int(work.width * scale))
        nh = max(120, int(work.height * scale))
...
def sign_pdf():
    pdf, original = save_file('file')
    out = add_text_overlay(pdf, original, request.form.get('signature','Signed'), 1, 360, 70, 22)
...
def _auto_config_tesseract():
    ...
    if not exe:
        return False, "Tesseract OCR app install nahi hai. Windows me UB-Mannheim Tesseract install karke PATH add karo, phir backend restart karo."
...
def _render_translated_pdf_image_mode(pdf_path, original, target_lang='Hindi', mode='layout'):
    ...
    out_images=[]
    zoom=float(request.form.get('quality','2') or 2)
...
def _render_translated_text_pdf(pdf_path, original, target_lang='Hindi'):
    # clean readable translated PDF fallback for files where same-layout replacement is not desired
    target=_target_code(target_lang)
    font_path=_find_font_for_lang(target)
    texts=pdf_text_pages(pdf_path)
    translated=_translate_batch(texts, target)
    out=OUTPUTS/(Path(original).stem+f'_translated_clean_{target}.pdf')
    W,H=A4
    c=canvas.Canvas(str(out), pagesize=A4)
    # reportlab may not shape Hindi perfectly, so this is fallback only; image mode is primary
    if font_path:
        try:
            from reportlab.pdfbase import pdfmetrics
            from reportlab.pdfbase.ttfonts import TTFont
            pdfmetrics.registerFont(TTFont('TMUnicode', font_path))
            c.setFont('TMUnicode', 11)
        except Exception: c.setFont('Helvetica',11)
    y=H-50
    for pi, page_text in enumerate(translated,1):
        c.setFont('Helvetica-Bold',13); c.drawString(40,y,f'Page {pi} - Translated to {target_lang}'); y-=25
        c.setFont('TMUnicode' if font_path else 'Helvetica', 11)
        for para in (page_text or '').split('\n'):
            for line in re.findall('.{1,85}(?:\\s+|$)', para) or ['']:
                c.drawString(45,y,line.strip()); y-=16
                if y<50: c.showPage(); y=H-50; c.setFont('TMUnicode' if font_path else 'Helvetica', 11)
            y-=4
        c.showPage(); y=H-50
    c.save(); return out
...
def translate_pdf():
    ...
    try:
        if mode in ('clean','text','readable'):
