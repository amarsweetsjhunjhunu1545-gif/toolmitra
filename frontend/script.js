...
const API = (location.hostname === 'localhost' || location.hostname === '127.0.0.1' || location.protocol === 'file:')
  ? 'http://127.0.0.1:5000'
  : 'https://toolmitra-backend.onrender.com';
...
function render(list=tools){const el=document.getElementById('toolGrid');if(!el)return;el.innerHTML=list.map((x)=>{const cat=toolCategory(x).toUpperCase();const isEd=x.t==='Edit PDF';return `<a href="${isEd?'javascript:openPdfEditor()':getSlug(x.t)+'.html'}" class="tool" style="text-decoration:none;color:inherit;display:block;"><span class="tag">${cat}</span><div class="icon">${x.i}</div><h3>${x.t}</h3><p>${x.d}</p><small class="toolAction">Open tool →</small></a>`}).join('')}
...
function openTool(i){
  if(tools[i].type==='editor'){openPdfEditor();return;}
